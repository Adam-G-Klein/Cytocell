...
public class FlitController : MonoBehaviour
{
    ...
    */
    ...
    public void checkPurged(Vector2[] killzone)
    {
        print(String.Format("{0} ran checkpurge", gameObject.name));
        if (UtilTools.InsidePolygon(killzone, (Vector2)transform.position))//coll.bounds.Intersects(manager.purgingZone.bounds))
        {
            print(String.Format("{0} called purge", gameObject.name));
            purge();
    ...
    private void deathCallback(){
        print("death callback on " + gameObject.name);
